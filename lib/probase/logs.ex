defmodule Probase.Logs do
  ...
  def query_data_final(search_params, page, size) do
    ...
    |> (fn %{entries:  entries} = result ->
        formated = Enum.map(entries, &Map.merge(&1, %{local_total_credit: local_total_credit(entries),
        local_total_debt: local_total_debt(entries)}))
        Map.put(result, :entries, formated)
    end).() 
  end
  
  defp local_total_credit(data), do: Enum.filter(data, &!is_nil(&1.lyc_cr_bal)) |> Enum.map(& &1.lyc_cr_bal |> Decimal.to_float) |> Enum.sum()

  defp local_total_debt(data), do: Enum.filter(data, &!is_nil(&1.lyc_dr_bal)) |> Enum.map(& &1.lyc_dr_bal |> Decimal.to_float) |> Enum.sum()
  ...
  defp composeul_isearch_filter(query, search_term) do
    ...
    |> where(
      [a, b],
      fragment("lower(?) LIKE lower(?)", fragment("concat(?, concat(' ', ?))", b.first_name, b.last_name), ^search_term)or
