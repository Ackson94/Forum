defmodule Probase.Notify do
  ...
  def create_chat(chat_params),
    do: Notification.changeset(%Notification{}, chat_params) |> Repo.insert()
  ...
  def alert_messager(),
    do: Repo.all(from(u in Notification, where: u.status == "PENDING", select: u)) |> Enum.count()

  def new_announcement(),
    do: Repo.all(from(u in Notification, where: u.status == "PENDING", select: u))
