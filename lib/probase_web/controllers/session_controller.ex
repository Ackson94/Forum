defmodule ProbaseWeb.SessionController do
    use ProbaseWeb, :controller
    alias ProbaseWeb.UserController
    alias Probase.{Auth, Logs, Repo}
    ...
    def create(conn, params) do
      with {:error, _reason} <- UserController.get_user_by_email(String.trim(params["email"])) do
        conn
        |> put_flash(:error, "Credentials not Matching")
        |> put_layout(false)
        |> render("index.html")
      else
        {:ok, user} ->
            ...
            {:ok, _} ->
              cond do
                ...
                true ->
                  ...
                  |> put_status(405)
                  |> put_layout(false)
                  |> render(ProbaseWeb.ErrorView, :"405")
              end
          end
      end
    end
