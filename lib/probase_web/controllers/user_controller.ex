defmodule ProbaseWeb.UserController do
  ...
  def dashboard(conn, _params),
    do:
      render(conn, "index.html",
        list_user: Accounts.list_tbl_users_acc(),
        chat_name: Chats.list_chats()
      )

  def search_query(conn, %{"name" => name}),
    do: render(conn, "search.html", name_forum: Chats.search_query(name))
  ...
  def upload_profile(conn, params) do
    ...
    |> Ecto.Multi.update(
      :profile,
      User.changeset(Accounts.get_user_profile(params["id"]), params)
    )
    ...
    with :error <- confirm_token(conn, user_params["id"]) do
      ...
      {:ok, user} ->
        ...
        |> Ecto.Multi.run(:log, fn %{update: _update} ->
          activity =
            "Modified user details with Email \"#{user.email}\" and First Name \"#{user.first_name}\""
  ...
  # ------------------ Password Reset ---------------------
  def new_password(conn, _params),
    do: render(conn, "change_password.html", page: %{first: "Settings", last: "Change password"})
