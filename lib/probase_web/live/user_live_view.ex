defmodule ProbaseWeb.UserLiveView do
  ...
  def render(assigns), do: ProbaseWeb.UserView.render("show.html", assigns)

  def mount(
        _params,
        %{"chat" => chat, "current_user" => current_user, "user_name" => user_name} = session,
        socket
      ) do
  ...
  def handle_event("new_message", %{"message" => %{"content" => ""}}, socket),
    do: {:noreply, socket}
  ...
  def handle_info(%{event: "presence_diff"}, socket = %{assigns: %{chat: chat}}),
    do: {:noreply, assign(socket, users: Presence.list_presences(live_view_topic(chat.id)))}

  def handle_info(%{event: "new_message", payload: state}, socket),
    do: {:noreply, assign(socket, state)}

  defp default_user_presence_payload(user),
    do: %{typing: false, first_name: user.first_name, email: user.email, user_id: user.id}
